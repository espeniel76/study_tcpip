...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        nSizeString = write(nSock, message, strlen(message));
        nSizeRecvString = 0;
        while (nSizeRecvString < nSizeString)
        {
            nRecvCnt = read(nSock, &message[nSizeRecvString], BUF_SIZE - 1);
            if (nRecvCnt == -1)
            {
                error_handling("read error");
            }
            nSizeRecvString += nRecvCnt;
        }
